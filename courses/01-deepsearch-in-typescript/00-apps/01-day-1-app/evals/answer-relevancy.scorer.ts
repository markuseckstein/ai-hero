...
function generateEvaluationStatementsPrompt({ output }: { output: string }) {
  return `
  Look at the given text and remove everything which belongs to the LLM's planning, reasoning or thought process.
  
  Use the remaining text, break it down into meaningful statements while preserving context and relationships.
...
export const AnswerRelevancy = createScorer<string, string, string>({
  ...
  scorer: async ({ input, expected, output }) => {
    ...
    const statements = statementsObj.statements;

    // Step 2: Score each statement for relevancy
    const { object: verdictsObj } = await generateObject({
      model: factualityModel,
